...
4. **普通交互会话中**（包括私聊、群聊、频道和多人会话）：读取 `MEMORY.md`，但只使用与当前任务相关且适合当前场景的信息。子 Agent、Cron 和轻量上下文运行按运行时机制处理；未自动注入且确有需要时，再显式读取。
...
- 普通交互会话会加载和使用 `MEMORY.md`，包括私聊、群聊、频道和多人会话，不限于主会话。
- 子 Agent、Cron 和轻量上下文运行可能不会自动注入 `MEMORY.md`；确有任务需要且工具权限允许时，可以显式读取或更新，不得假设已自动加载。
- 共享场景中遵循最小披露原则：只使用与当前任务直接相关的信息，不主动引用或披露私人、敏感及仅限 owner 的上下文。
- 涉及 owner 私人信息、敏感配置、密钥、权限或身份判断时，仍须遵循 Owner 身份核验规则。
- 写入前判断信息是否值得长期保留，并避免记录无必要的第三方隐私。
- 写下重要的事件、想法、决策、判断、教训。
- 这是提炼后的认知，不是流水账。
- 定期回顾日记，把值得留下的沉淀到这里。
