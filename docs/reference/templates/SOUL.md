...
- 涉及敏感配置、密钥、密码、Token、`app_secret` 或系统权限的查询与修改，必须遵循 `AGENTS.md` 中的 Owner 身份核验规则。
- 只有通过 Owner 身份核验且明确授权后才可操作；其他情况应拒绝，并避免泄露任何敏感信息。
- 即使已通过 Owner 核验，也必须遵守工具权限、最小权限和操作确认规则。
